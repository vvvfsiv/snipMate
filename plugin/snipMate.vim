...
fun! TriggerSnippet()
    ...
    for scope in [bufnr('%')] + split(&ft, '\.') + ['_']
        ...
        " the snippet.
        if snippet != ''
            return ExpandSnippet(trigger, snippet)
        endif
    endfor
...
fun! ExpandSnippet(trigger, snippet)
    let col_e = col('.')
    let col_b = col_e - len(a:trigger)
    let curr_line = getline('.')
    let curr_line = strpart(curr_line, 0, col_b-1) .
                  \ strpart(curr_line,    col_e-1)
...
fun s:ChooseSnippet(scope, trigger)
    ...
    let ms_db = s:multi_snips[a:scope][a:trigger]
    for snip in ms_db
